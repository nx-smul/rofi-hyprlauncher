...
SHORTCUT_SCRIPT="$SCRIPT_DIR/get_shortcut.py"
...
update-game-entries() {
  ...
  for library in $(steam-libraries); do
    ...
    for manifest in "$library"/steamapps/appmanifest_*.acf; do
      ...
      if [ -z $update ] && [ -f "$entry" ]; then
        [ -z $quiet ] && echo "Skipping update for $entry (Already exists)"
        continue
      fi
      ...
      if [ ! -f "$boxart" ]; then
        [ -z $quiet ] && echo "Skipping $title (No boxart found)"
        continue
      fi
      ...
      [ -z $quiet ] && echo -e "Generating $entry\n[ $title ] with boxart: $boxart"
      desktop-entry "$appid" "$title" "$boxart" >"$entry"

      # Insert a blank line to separate output for better readability
      echo ""
    done
  done
  ...
  while IFS=, read -r appid name exe; do
    ...
    # Don't update existing entries unless doing a full refresh
    if [ -z $update ] && [ -f "$entry" ]; then
      [ -z $quiet ] && echo "Skipping update for $entry (Shortcut already exists for $name)"
      continue
    fi

    if [ -f "$boxart" ]; then
      [ -z $quiet ] && echo -e "Generating $entry\n[ $name ] with boxart: $boxart"
      desktop-entry "$appid" "$name" "$boxart" >"$entry"
    else
      [ -z $quiet ] && echo "Skipping $name (No boxart found)"
    fi
